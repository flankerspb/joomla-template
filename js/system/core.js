	...
	Joomla.renderMessages = function(messages)
  {
		...
		for(var type in messages)
		{
			...
			switch(type)
			{
				case 'primary':
				case 'info':
					// notify.message = '<i class="uk-icon-check uk-margin-smal-right"></i>';
          ...
          header = 'NOTICE';
			...
			console.log(header);
			console.log(Joomla.JText._(header));
			if(header)
			{
				notify.message += '<b>' + Joomla.JText._(header) + '</b>';
			}
			
			// Array of messages of this type
			var typeMessages = messages[type].reverse();
			
			for(var i = 0; i < typeMessages.length; i++)
			{
				notify.message += '<div>' + typeMessages[i] + '</div>';
			}
