...
var JFormValidator = function() {
  ...
  handleResponse = function(state, $el) {
    ...
    if (state === false) {
      $el.addClass('uk-form-danger').attr('aria-invalid', 'true');
      if ($label) {
        $label.addClass('uk-text-danger');
      }
    } else {
      $el.removeClass('uk-form-danger').attr('aria-invalid', 'false');
      if ($label) {
  ...
  isValid = function(form) {
    ...
    if(!valid && invalid.length > 0)
    {
      ...
      message = '<p>' + Joomla.JText._('JLIB_FORM_FIELD_INVALID') + '</p><ul>';
      ...
      for (i = 0; i < invalid.length; i++)
      {
        ...
        if(label)
        {
          message += '<li>' + label.text().replace('*', '') + '</li>';
        }
      }
