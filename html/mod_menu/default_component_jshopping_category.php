...
if(!function_exists('recursiveCatsDefault'))
{
	function recursiveCatsDefault(&$cats, $id)
	{
		static $level = 0;
		
		$html = $level ? '<ul class="fl-level-'.($level + 1).'">' : '<ul class="uk-nav uk-nav-dropdown">';
		...
		foreach($cats[$id] as $cat)
		{
			if($cats[$cat->id])
			{
				...
				$level++;
				$html .= recursiveCatsDefault($cats, $cat->id);
				$level--;
			}
			else
			{
